...
/*   Updated: 2018/04/02 20:24:39 by jjaniec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_skip_direntries_alphacmp(t_dir_entry **ptr, \
					t_dir_entry **new, t_dir_entry **prev, int rev)
{
	if (rev && (*new) && (*ptr))
					...
					t_opt *opts)
{
	...
	if ((*__OS__ == 'L' && !(opts && opts->t)) || \
		...
		ft_skip_direntries_alphacmp(&ptr, &new, &prev, \
			(opts) ? (opts->r) : (0));
	else
